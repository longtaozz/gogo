...
public class HttpUtil {
     ...
     * @param myNetCall
     */
    public void getDataAsyncFile(String url, final MyNetCall myNetCall) {
        //1 构造Request
        Log.d("URLxxxxxxxxxx", url);
        Request.Builder builder = new Request.Builder().header("x-token", Web.getToken());
        Request request = builder
        ...
        call.enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                if (response.code() == 200) {  // 0 成功   1 失败
                    myNetCall.success(0, response, response.message(), token);
                } else if (response.code() == 404) {
                    myNetCall.success(404, null, response.message(), token);
                } else {
                    myNetCall.success(1, null, response.message(), token);
                }
            }
        });
    }

package com.zt.capacity.jinan_zwt.request.push;
...
import com.zt.capacity.jinan_zwt.bean.PushRecords;
...
public class PushManager extends Web implements IPushManager {
    ...
    public static IPushManager getInterface(Integer state) {
        if (state == 1) {
            //检查用户是否能接收通知
            pushManager = new PushManager(JN_ZWT_Url.HWTOKENCHECK);
            return (IPushManager) pushManager;
        }
        if (state == 2) {
            //请求通知记录
            pushManager = new PushManager(JN_ZWT_Url.GETPUSHRECORDVO);
            return (IPushManager) pushManager;
        }
        if (state == 3) {
            //修改阅读状态
            pushManager = new PushManager(JN_ZWT_Url.UPPUSHRECORD);
            return (IPushManager) pushManager;
        }
        if (state == 4) {
            //查询未读数量
            pushManager = new PushManager(JN_ZWT_Url.GETPUSHRECORDCOUNT);
            return (IPushManager) pushManager;
        }
        return null;
    }

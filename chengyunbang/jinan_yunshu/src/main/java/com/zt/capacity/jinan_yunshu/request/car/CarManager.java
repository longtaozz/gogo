...
import com.zt.capacity.jinan_yunshu.bean.DataActionBeans;
import com.zt.capacity.jinan_yunshu.data.JN_YunShu_Url;
import com.zt.capacity.jinan_yunshu.request.instruction.IInstructionManager;
...
public class CarManager extends Web implements IInstructionManager {
    Gson gson = new GsonBuilder().disableHtmlEscaping().create();

    public static IInstructionManager getInterface(Integer state) {
        CarManager instructionManager = null;
        if (state == 1) {
            //查询指令历史记录
            instructionManager = new CarManager(JN_YunShu_Url.FINDACTIONLIS);
        }
     ...
     * @param
     */
    protected CarManager(String url) {
        super(url, 3);
    }
